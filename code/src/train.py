...
from lib.datasets1 import get_fix_data
...
def main(args):
    ...
    fixed_img, fixed_sent, fixed_words, fixed_asp1, fixed_asp2, fixed_asp3, fixed_z = \
                                    get_fix_data(train_dl, valid_dl, text_encoder, args)                                #1
    if (args.multi_gpus==True) and (get_rank() != 0):
        None
    else:
    ...
    for epoch in range(start_epoch, args.max_epoch, 1):
        ...
        if epoch%gen_interval==0:
            sample(fixed_z, fixed_sent, fixed_asp1, fixed_asp2, fixed_asp3, fixed_words, netG, args.multi_gpus,
                   epoch, args.img_save_dir, writer)                                                                    #1
            torch.cuda.empty_cache()
        # test
